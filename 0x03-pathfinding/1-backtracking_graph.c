 ...
 */
int backtrack_graph_helper(queue_t **queue, int *visited, vertex_t const *cur,
			   vertex_t const *target)
{
	...
	while (edge)
	{
		ret = backtrack_graph_helper(queue, visited, edge->dest, target);
		if (ret)
		{
			queue_push_front(*queue, strdup(cur->content));
			return (1);
			    ...
			    vertex_t const *target)
{
	...
	if (!q)
		return (NULL);
	s = backtrack_graph_helper(&q, visited, start, target);
	free(visited);
	if (!s)
	{
		queue_delete(q);
